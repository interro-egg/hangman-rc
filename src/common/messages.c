...
ssize_t serializeRSGMessage(RSGMessage *msg, char* outBuffer) {
  return sprintf(outBuffer, "RSG %s %u %u\n", msg->status, msg->n_letters, msg->max_errors);
}
...
RSGMessage *deserializeRSGMessage(char *inBuffer) {
  ...
  free(status);
  msg->n_letters = readInt(inBuffer);
  msg->max_errors = readInt(inBuffer);
  if (msg->n_letters == NULL || msg->max_errors == NULL) {
    destroyRSGMessage(msg);
    return NULL;
  }
  return msg;
...
PLGMessage *deserializePLGMessage(char *inBuffer){
  ...
  if (sscanf(inBuffer, "%6s %1c %1u", msg->PLID, msg->letter, msg->trial) != 3) {
    destroyPLGMessage(msg);
    return NULL;
  }
  return msg;
